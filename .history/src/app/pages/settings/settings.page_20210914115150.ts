...
export class SettingsPage implements OnInit {
  ...
  private async getDevice(): Promise<void> {
    const data: ReportsDevicesPayload = { active: ACTIVE, status_id: DEFAULT, deviceType: DEVICE_TYPE, deviceSerialNum: this.uuid };
  ...
  public async updateDevice(): Promise<void> {
    ...
    const addDevicesPayload: SettingsReportRequest = { data: data };
    (await this.httpService.updateDevice(addDevicesPayload)).pipe(take(1), finalize(() => this.loadingService.dismissLoading()))
      .subscribe(
        res => {
          if (res.statusCode != OK) {
            console.error("status code: " + res.statusCode);
