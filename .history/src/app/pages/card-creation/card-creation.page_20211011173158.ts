...
export class CardCreationPage implements OnInit {
    ...
    switch (this.isMagneticEncoderExist) {
      case true:
        if (this.isDriver && !this.isEmployee) {
          this.createComboCard(CardType.DRIVER_CARD)
        } else if (!this.isDriver && this.isEmployee) {
          this.createEmployeeCard()
        } else if (this.isDriver && this.isEmployee) {
          this.createComboCard(CardType.EMPLOYEE_CARD)
        }
        break;
      case false:
  ...
  private async createComboCard(cardType: string): Promise<void> {
    ...
    if (rmsg.status === ReaderStatus.SUCCESS) {
      if (await this.writeDriverCard())
        await this.printCombo(cardType)
    }
  ...
  public async printCombo(cardType: string): Promise<void> {
  ...
  public async createCardWitoutPrinting() {
