...
export class ProgressProcessComponent implements OnDestroy {
  ...
  public next() {
    ...
    let msg: any = new Promise(async resolve => { this.readerService.messages.subscribe(msg =>  resolve(msg) )})
    if (!msg)
      msg = new Promise(async resolve => {this.printerService.messages.subscribe(msg => resolve(msg)) })
