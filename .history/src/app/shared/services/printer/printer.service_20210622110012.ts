...
export class PrinterService {
  ...
  public moveTo(station: string): Promise<ReceivePrinterMessage> {
    return new Promise(async resolve => {
      this.messages.next({
        ...
        printerName: (await this.storeService.get("printerSettings") as OtherDevicesPayload).deviceSerNum,
        station: station
      })
  ...
  public printCombo(firstName: string, lastName: string, isTakenAPicture: string, cardType: string, track2: string): Promise<ReceivePrinterMessage> {
    return new Promise(async resolve => {
      this.messages.next({
