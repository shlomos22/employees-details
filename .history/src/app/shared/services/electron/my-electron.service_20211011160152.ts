...
export class MyElectronService {
  ...
  constructor(
    ...
    if (!isDevMode()) {
      this.rootPath = `${this.remote.app.getPath('exe')}\\resources\\extraResources`
    }
  ...
  private async installReaderFile() {
    const command = `echo Y | xcopy "${this.rootPath}\\${READER_JAR}" "${READER_PATH}"`
    console.log(command);
    return this.runScript(command);
  }

  private async installReaderSettingsFile() {
    const command = `echo Y | xcopy "${this.rootPath}\\${READER_SETTINGS}" "${READER_PATH}"`
    console.log(command);
    return this.runScript(command);
  }

  private async installPrinterFile() {
    const command = `echo Y | xcopy "${this.rootPath}\\${PRINTER_JAR}" "${PRINTER_PATH}"`
    console.log(command);
    return this.runScript(command);
  }

  private async installPrinterSettingsFile() {
    const command = `echo Y | xcopy "${this.rootPath}\\${PRINTER_XML}" "${PRINTER_PATH}"`
    console.log(command);
    return this.runScript(command);
  }
