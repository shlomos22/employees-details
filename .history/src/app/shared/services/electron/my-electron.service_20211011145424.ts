...
export class MyElectronService {
  ...
  private async installReaderFile() {
    const command = `xcopy /d "${this.rootPath}\\${READER_JAR}" "${READER_PATH}\\${READER_JAR}"`
    console.log(command);
    return this.runScript(command);
  }

  private async installReaderSettingsFile() {
    const command = `xcopy /d "${this.rootPath}\\${READER_SETTINGS}" "${READER_PATH}\\${READER_SETTINGS}"`
    console.log(command);
    return this.runScript(command);
  }

  private async installPrinterFile() {
    const command = `xcopy /d "${this.rootPath}\\${PRINTER_JAR}" "${PRINTER_PATH}\\${PRINTER_JAR}"`
    console.log(command);
    return this.runScript(command);
  }

  private async installPrinterSettingsFile() {
    const command = `xcopy /d "${this.rootPath}\\${PRINTER_XML}" "${PRINTER_PATH}\\${PRINTER_XML}"`
    console.log(command);
    return this.runScript(command);
  }
