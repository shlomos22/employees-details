...
import { PRINTER_PATH, JAVA_PATH, PRINTER_JAR, PRINTER_XML, READER_JAR, READER_PATH, READER_SETTINGS, JAVA_PATH_PROD, CONFIG_PROPERTIESL } from '../../constants/files';
...
export class MyElectronService {
  ...
  private async installPrinterPropertiesFile() {
    const command = `echo Y | xcopy "${this.rootPath}\\${CONFIG_PROPERTIESL}" "${PRINTER_PATH}" /S /Y /D`
  }
