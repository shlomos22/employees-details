...
export class MyElectronService {
  ...
  private async installReaderFile() {
    const command = `xcopy  ${this.rootPath}\\${READER_JAR} ${READER_PATH}\\${READER_JAR} /F /Y`
    console.log(command);
    return this.runScript(command);
  }

  private async installReaderSettingsFile() {
    const command = `xcopy  ${this.rootPath}\\${READER_SETTINGS} ${READER_PATH}\\${READER_SETTINGS} /F /Y`
    console.log(command);
    return this.runScript(command);
  }

  private async installPrinterFile() {
    const command = `xcopy  ${this.rootPath}\\${PRINTER_JAR} ${PRINTER_PATH}\\${PRINTER_JAR} /F /Y`
    console.log(command);
    return this.runScript(command);
  }

  private async installPrinterSettingsFile() {
    const command = `xcopy  ${this.rootPath}\\${PRINTER_XML} ${PRINTER_PATH}\\${PRINTER_XML} /F /Y`
    console.log(command);
    return this.runScript(command);
  }
