...
export class MyElectronService {
  ...
  private async installReaderFile() {
    const command = `echo Y | xcopy /d "${this.rootPath}\\${READER_JAR}" "${READER_PATH}"`
    console.log(command);
    return this.runScript(command);
  }

  private async installReaderSettingsFile() {
    const command = `echo Y | xcopy /d "${this.rootPath}\\${READER_SETTINGS}" "${READER_PATH}"`
    console.log(command);
    return this.runScript(command);
  }

  private async installPrinterFile() {
    const command = `echo Y | xcopy /d "${this.rootPath}\\${PRINTER_JAR}" "${PRINTER_PATH}"`
    console.log(command);
    return this.runScript(command);
  }

  private async installPrinterSettingsFile() {
    const command = `echo Y | xcopy /d "${this.rootPath}\\${PRINTER_XML}" "${PRINTER_PATH}"`
    console.log(command);
    return this.runScript(command);
  }
