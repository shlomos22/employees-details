...
export class MyElectronService {
  ...
  private async createReaderFolder() {
    const command = `mkdir -p ${READER_PATH}`
    return this.runScript(command)
  }

  private async createPrinterFolder() {
    const command = `mkdir -p ${PRINTER_PATH}`
    return this.runScript(command)
  }

  private async installReaderFile() {
    const command = `xcopy  ${this.rootPath}\\${READER_JAR} ${READER_PATH}\\${READER_JAR} /F /R /Y /I`
    console.log(command);
  ...
  private async installReaderSettingsFile() {
    const command = `xcopy  ${this.rootPath}\\${READER_SETTINGS} ${READER_PATH}\\${READER_SETTINGS} /F /R /Y /I`
    return this.runScript(command);
  }

  private async installPrinterFile() {
    const command = `xcopy  ${this.rootPath}\\${PRINTER_JAR} ${PRINTER_PATH}\\${PRINTER_JAR} /F /R /Y /I`
    return this.runScript(command);
  }

  private async installPrinterSettingsFile() {
    const command = `xcopy  ${this.rootPath}\\${PRINTER_XML} ${PRINTER_PATH}\\${PRINTER_XML} /F /R /Y /I`
    return this.runScript(command);
  }
